...
Using CRA allows fast initial setup, but adds some "magic" to the repo. We felt this was a good tradeoff for initial development. CRA also offers `eject` which allows for it to be removed as a dependancy (though doing so also removes many of it's benefits, such as not maintaining the webpack config). There is also a large dev community for CRA. 
...
- `-` Low flexibility (if we run into a unique react issue we need to solve, we may need to `eject`)
